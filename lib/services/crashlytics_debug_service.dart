...
class CrashlyticsDebugService {
  ...
  static Future<void> testCrashlyticsDirectly() async {
    try {
      ...
      // 1. Verificar si Crashlytics está habilitado (método alternativo)
      print('[CrashlyticsDebug] 📊 Crashlytics configurado para debug');

      // 2. Registrar un log
      await FirebaseCrashlytics.instance
          .log('Test directo de Crashlytics - ${DateTime.now()}');
  ...
  static Future<void> checkFirebaseConfig() async {
    try {
      ...
      print(
          '[CrashlyticsDebug] 📱 Firebase App: ${FirebaseCrashlytics.instance.app.name}');

      // Verificar configuración de Crashlytics (método alternativo)
      print('[CrashlyticsDebug] 📊 Crashlytics configurado para debug');

      // Habilitar Crashlytics explícitamente
