...
class _ButtonRememberAccount extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialButton(
      ...
      minWidth: 300,
      child: CustomStyleButton(text: loginForm.isLoading ? 'Recuperando datos...' : 'Recuperar contraseña'),
      onPressed: loginForm.isLoading
        ? null
        : () async {
          if (!loginForm.isValidForm()) return;
          rememberAccount(context, loginForm, loginService, inspeccionService);
        },
    );
  }
          ...
          builder: (context) => Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, int i) => ListTile(
                ...
                onTap: () async {
                  final resSendEmail = await loginService.sendEmailRememberData(empresas[i]);
                  await showDialog(
                    context: context,
                    builder: (context) => AlertDialog(
                      title: Icon(Icons.warning, color: Colors.orange,),
                      content: Text('${resSendEmail['message']} con los datos de tu cuenta', textAlign: TextAlign.center),
                      actions: [
                        TextButton(onPressed: () => Navigator.pop(context, true), child: Text('Comprendo', style: TextStyle(color: Colors.green))),
                      ],
                    )
                  );
                  Navigator.pop(context);
                  loginService.pageController.previousPage(duration: Duration(microseconds: 1000), curve: Curves.bounceOut);
                },
              ),
            ),
          )
